...
class ConstraintTest(unittest.TestCase):
    def testConstraintsAndObjective(self):
        ...
        self.assertAlmostEqual(solution[x2], 0.0)
        self.assertAlmostEqual(solution.objective, 2.0)
                
        # Test maximizing
        solution = solver.maximize(objective=x1+5*x2)
        self.assertAlmostEqual(solution[x1], 0.0)
        self.assertAlmostEqual(solution[x2], 1.0)
        self.assertAlmostEqual(solution.objective, 5.0)

        # Test minimizing
        solution = solver.minimize(objective=x1+0.25*x2)
        self.assertAlmostEqual(solution[x1], 0.0)
        self.assertAlmostEqual(solution[x2], 1.0)
        self.assertAlmostEqual(solution.objective, 0.25)
...
class ConstraintRemovalTest(unittest.TestCase):
    ...
    def testBasicConstraintRemoval(self):
        ...
        self.assertAlmostEqual(1.0, self.solver.maximize(objective=self.x1+self.x2).objective)

        # Remove c2 and test again
        self.solver -= self.c2
        self.assertAlmostEqual(2.0, self.solver.maximize(objective=self.x1+self.x2).objective)
